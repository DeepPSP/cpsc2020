...
class CPSC2020(Dataset):
    ...
    def _preprocess_one_record(
        self, rec: Union[int, str], config: dict, force_recompute: bool = False, verbose: int = 0
    ) -> NoReturn:
        """finished, checked,
    ...
    def _slice_one_record(
        self, rec: Union[int, str], force_recompute: bool = False, update_segments_json: bool = False, verbose: int = 0
    ) -> NoReturn:
        """finished, checked,
        ...
        beat_ann = list(
            repeat({"SPB_indices": np.array([], dtype=int), "PVC_indices": np.array([], dtype=int)}, len(non_premature))
        )
    ...
    def plot_seg(
        self, seg: str, ticks_granularity: int = 0, rpeak_inds: Optional[Union[Sequence[int], np.ndarray]] = None
    ) -> NoReturn:
        """finished, checked,
