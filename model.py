...
class ECG_CRNN_CPSC2020(ECG_CRNN):
    ...
    def __init__(
        self, classes: Sequence[str], n_leads: int, input_len: Optional[int] = None, config: Optional[ED] = None
    ) -> NoReturn:
        """finished, checked,
    ...
    @torch.no_grad()
    def inference(
        self, input: Union[np.ndarray, Tensor], class_names: bool = False, bin_pred_thr: float = 0.5
        ...
        if class_names:
            ...
            for row_idx in range(len(pred)):
                pred.at[row_idx, "bin_pred"] = np.array(self.classes)[np.where(bin_pred == 1)[0]].tolist()
...
class ECG_SEQ_LAB_NET_CPSC2020(ECG_SEQ_LAB_NET):
    ...
    @torch.no_grad()
    def inference(
        self, input: Union[np.ndarray, Tensor], bin_pred_thr: float = 0.5, rpeak_inds: Optional[List[np.ndarray]] = None
        ...
        SPB_intervals = [
            mask_to_intervals(seq * rpeak_mask[idx], 1) for idx, seq in enumerate(bin_pred[..., self.classes.index("S")])
        ]
        SPB_indices = [
            [self.reduction * (itv[0] + itv[1]) // 2 for itv in l_itv] if len(l_itv) > 0 else [] for l_itv in SPB_intervals
        ]
        PVC_intervals = [
            mask_to_intervals(seq * rpeak_mask[idx], 1) for idx, seq in enumerate(bin_pred[..., self.classes.index("V")])
        ]
        PVC_indices = [
