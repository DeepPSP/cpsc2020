...
from cfg import BaseCfg, FeatureCfg, PreprocCfg
...
class CPSC2020Reader(object):
    ...
    def preprocess_data(self, rec: Union[int, str], preproc: List[str]) -> NoReturn:
        ...
        pps["rpeaks"] = pps["rpeaks"][
            np.where((pps["rpeaks"] >= config.beat_winL) & (pps["rpeaks"] < len(pps["filtered_ecg"]) - config.beat_winR))[0]
        ]
    ...
    def compute_features(
        self, rec: Union[int, str], features: List[str], preproc: List[str], augment: bool = True, save: bool = True
    ) -> np.ndarray:
        """finished, checked,
    ...
    def load_features(
        ...
        if os.path.isfile(feature_fp) and not force_recompute:
            ...
            feature_mat = self.compute_features(rec, features, preproc, augment, save=True)
        return feature_mat
    ...
    def _ann_to_beat_ann(
        ...
        for idx in range(len(split_indices) - 1):
            ...
            p["ann"] = {
                k: v[np.where((v >= p["rpeaks"][0] - bias_thr - 1) & (v < p["rpeaks"][-1] + bias_thr + 1))[0]]
        ...
        ann_matched = {k: np.concatenate([item["ann_matched"][k] for item in result]) for k in ann.keys()}
        ann_not_matched = {k: [a for a in v if a not in ann_matched[k]] for k, v in ann.items()}
        # print(f"rec = {rec}, ann_not_matched = {ann_not_matched}")
        beat_ann = np.concatenate([item["beat_ann"] for item in result]).astype("<U1")

        augmented_rpeaks = np.concatenate(
            (rpeaks, np.array(ann_not_matched["SPB_indices"]), np.array(ann_not_matched["PVC_indices"]))
        )
        beat_ann = np.concatenate(
            (
                beat_ann,
                np.array(["S" for _ in ann_not_matched["SPB_indices"]], dtype="<U1"),
                np.array(["V" for _ in ann_not_matched["PVC_indices"]], dtype="<U1"),
            )
        )
        ...
        valid_indices = np.where(
            (augmented_rpeaks >= BaseCfg.beat_winL) & (augmented_rpeaks < len(raw_sig) - BaseCfg.beat_winR)
        )[0]
    ...
    def _get_ann_name(self, rec: Union[int, str]) -> str:
        ...
        if isinstance(rec, int):
            ...
            assert rec in self.all_annotations + self.all_records, "rec should be one of {} or one of {}".format(
                self.all_records, self.all_annotations
            )
        ...
        for subset in ["train", "test"]:
            for rec in split_rec[subset]:
                ...
                feature_mat = self.load_features(
                    rec, features=features, preproc=preproc, augment=augment, force_recompute=False
                )
                beat_ann = self.load_beat_ann(
                    rec, preproc=preproc, augment=augment, return_aux_data=True, force_recompute=False
                )
    ...
    def plot(
        ...
        for idx in range(nb_lines):
            ...
            for t in seg_spb:
                ax.axvspan(max(secs[0], t - 0.05), min(secs[-1], t + 0.05), color=self.palette["spb"], alpha=0.5)
            for t in seg_pvc:
                ax.axvspan(max(secs[0], t - 0.05), min(secs[-1], t + 0.05), color=self.palette["pvc"], alpha=0.5)
            if len(patches) > 0:
    ...
    for idx_r, r in enumerate(rpeaks):
        ...
        for idx_a, a in enumerate(_ann["PVC_indices"]):
            if abs(r - a) < bias_thr:
                ...
                break
    ann_matched = {k: np.array([a for a in v if a not in _ann[k]], dtype=int) for k, v in ann.items()}
    retval = dict(ann_matched=ann_matched, beat_ann=beat_ann)
    return retval
